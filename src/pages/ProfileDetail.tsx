...
import { getProfileById, getProfileMatches } from '../api/client';
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 animate-slide-up">
        ...
        <div className="mb-6">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
            <div className="flex-1">
              <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">{profile.name}</h1>
          ...
          {profile.contact_email && (
            <div className="flex items-start">
              ...
              <div>
                ...
                <a href={`mailto:${profile.contact_email}`} className="text-green-600 hover:text-green-700 font-medium">
                  {profile.contact_email}
                </a>
              </div>
            </div>
      ...
      <div className="card p-6 md:p-8">
        ...
        {!loadingMatches && matches.length > 0 && (
          <div className="grid gap-4">
            {matches.map((vacancy, index) => (
              <Link
                key={vacancy.id}
                to={`/vacancies/${vacancy.id}`}
                className="border-2 border-gray-200 rounded-xl p-6 hover:border-primary-300 hover:shadow-lg transition-all duration-200 group"
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className="ml-4 px-3 py-1 bg-primary-100 text-primary-700 text-xs font-semibold rounded-full">
                        {Math.round(vacancy.confidence * 100)}%
                      </span>
                    </div>
                    <p className="text-gray-600 mb-4 line-clamp-2 leading-relaxed">{vacancy.description}</p>
                    <div className="flex flex-wrap gap-3 text-sm">
                      <span className="inline-flex items-center px-3 py-1 bg-primary-50 text-primary-700 rounded-lg font-medium">
                  ...
                  <svg className="h-6 w-6 text-gray-400 group-hover:text-primary-600 transform group-hover:translate-x-1 transition-all ml-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
